...
namespace DoAn.Net
{
    public partial class Form_MoKhoaThe : Form
    {
        int maDG;
        DataConnect dataConnect = new DataConnect();
        public Form_MoKhoaThe(int pmaDG)
        {
            maDG = pmaDG;
            InitializeComponent();
        }

        private void Form_MoKhoaThe_FormClosing(object sender, FormClosingEventArgs e)
        {
            DialogResult r;
        ...
        private void xacNhanPhat_Button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (float.TryParse(txt_SoTien.Text, out float tien) && tien > 0)
                {
                    ...
                    if (result > 0)
                        MessageBox.Show("Mở khóa thẻ thành công");
                    else MessageBox.Show("Mở khóa thẻ thất bại!");
                }
                else
                {
                    MessageBox.Show("Có lỗi khi Mở khóa thẻ!");
                }
                txt_SoTien.Clear();
                txt_GhiChu.Clear();
            }
